...
import { Check, ArrowRight, Minus, Plus, Trash2 } from 'lucide-react';
...
const BundleBuilderPage = () => {
  ...
  const calculateTotal = () => {
    ...
    let discount = 0;
    const totalItems = selectedOils.length + selectedAddons.length;
    if (totalItems >= 3) discount = 60;
    else if (totalItems >= 2) discount = 30;
    
    if (deliveryType === 'subscription') {
      discount += Math.round(subtotal * 0.1); // 10% subscription discount
    }
  ...
  const handleAddBundle = async () => {
    ...
    for (const item of allItems) {
      await addToCart({
        ...
        is_subscription: deliveryType === 'subscription',
        frequency: deliveryType === 'subscription' ? frequency : null,
        price: item.price,
        name: item.name,
        image: item.images?.[0] || ''
      });
  ...
  const steps = [
    ...
    { num: 3, title: 'Delivery', desc: 'Choose frequency' },
  ];
  ...
  return (
    <>
    <SEO
      title="Build Your Custom Oil Bundle – Save More"
      description="Create your personalised cold-pressed oil bundle. Mix and match from groundnut, coconut, sesame, mustard and more. Save up to 15% on custom combos with free shipping above ₹999."
      canonical="/bundle"
      schema={{
        ...
        "description": "Customise your cold-pressed oil bundle from Krishi Foods and save more.",
        "breadcrumb": {
          "@type": "BreadcrumbList",
          "itemListElement": [
      ...
      <div className="bg-[#2D5016] py-12">
        <div className="container-krishi">
          ...
          <p className="text-[#F5EDD6]/80 text-center mt-2">
            Pick your favorites and save up to ₹60
          </p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2">
                  ...
                  {oils.map((oil) => {
                    ...
                    return (
                      <button
                        ...
                        className={`product-card text-left transition-all ${
                          isSelected ? 'ring-2 ring-[#2D5016]' : ''
                        }`}
                        data-testid={`oil-${oil.slug}`}
                      >
                        <div className="relative aspect-square overflow-hidden">
                          <img 
                            src={oil.images?.[0] || 'https://images.unsplash.com/photo-1474979266404-7eaacbcd87c5?w=500'} 
                            alt={oil.name}
                  ...
                  {addons.map((addon) => {
                    ...
                    return (
                      ...
                      >
                        <div className="relative aspect-square overflow-hidden">
                          <img 
                            src={addon.images?.[0] || 'https://images.unsplash.com/photo-1474979266404-7eaacbcd87c5?w=500'} 
                            alt={addon.name}
                            className="w-full h-full object-cover"
                          />
                          {isSelected && (
            ...
            {step === 3 && (
              <div data-testid="step-3-content">
                <h2 className="heading-h3 mb-6">Choose Delivery Type</h2>
                
                <div className="space-y-4">
                  <button
                    onClick={() => setDeliveryType('one-time')}
                    className={`w-full p-6 rounded-2xl border-2 text-left transition-all ${
                      deliveryType === 'one-time' ? 'border-[#2D5016] bg-[#2D5016]/5' : 'border-[#2D5016]/20'
                    }`}
                    data-testid="delivery-one-time"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-[#1A2F0D]">One-time Purchase</p>
                        <p className="text-sm text-[#4A5D3F]">Single delivery at your convenience</p>
                      </div>
                  ...
                  <button
                    ...
                    className={`w-full p-6 rounded-2xl border-2 text-left transition-all ${
                      deliveryType === 'subscription' ? 'border-[#2D5016] bg-[#2D5016]/5' : 'border-[#2D5016]/20'
                    }`}
                    data-testid="delivery-subscription"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <p className="font-semibold text-[#1A2F0D]">Subscribe & Save 10%</p>
                        <p className="text-sm text-[#4A5D3F]">Automatic monthly deliveries</p>
                      </div>
                    ...
                    {deliveryType === 'subscription' && (
                      <div className="flex gap-3 mt-4 pt-4 border-t border-[#2D5016]/10">
                ...
                <div className="mt-8 flex justify-between">
                  <button onClick={() => setStep(2)} className="btn-secondary" data-testid="back-step-3">
                    Back
                  </button>
                  <button 
